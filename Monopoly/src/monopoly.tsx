...
import { MonopolySettings, MonopolyModes } from "./assets/types";
function App({ socket, name, server }: { socket: Socket; name: string; server: Server | undefined }) {
    ...
    useEffect(() => {
        ...
        function playerMoveGENERATOR(final_position: number, _xplayer: Player, get200whengo: boolean = true, afterFinished?: () => void, adding: boolean = true) {
                    ...
                    if (i < sum_moves) {
                        ...
                        if (_xplayer.position == 0 && get200whengo) {
                            ...
                            if (_xplayer.id === socket.id) {
                                if (settings !== undefined && settings.notifications === true) notifyRef.current?.message(`${200} of money is added to the account`, "info", 2, () => {}, false);
                ...
                if (args.pJson.id !== socket.id) {
                    if (clients.size > 2) {
                        ...
                        if (clients.has(socket.id)) {
                            ...
                            notifyRef.current?.dialog(
                                (close_func, createButton) => ({
                                    innerHTML: `<h3> ${name} WON! </h3> <p> ${name} won with the balance of ${clients.get(socket.id)?.balance ?? 0} </p>`,
                                    buttons: [
                                        createButton("PLAY ANOTHER GAME", () => {
                                            close_func();
                                            document.location.reload();
                        ...
                        if (proprety.id === "communitychest" || proprety.id === "chance") {
                            ...
                            engineRef.current?.setStreet({
                                ...
                                onResponse: (b, info) => {
                                    ...
                                    if (b === "buy") {
                                        if (settings !== undefined && settings.notifications === true) notifyRef.current?.message(`${(proprety?.price ?? 0) * 1} of money is deducted from the account`, "info", 2, () => {}, false);
                                        ...
                                        if (_info.state === 5) {
                                            if (settings !== undefined && settings.notifications === true) notifyRef.current?.message(`${proprety.ohousecost ?? 0} of money is deducted from the account`, "info", 2, () => {}, false);
                                            localPlayer.balance -= proprety.ohousecost ?? 0;
                                            engineRef.current?.applyAnimation(1);
                                        } else {
                                            if (settings !== undefined && settings.notifications === true) notifyRef.current?.message(`${proprety.housecost ?? 0} of money is deducted from the account`, "info", 2, () => {}, false);
                                        ...
                                        for (const p of players) {
                                            for (const prp of p.properties) {
                                                if (prp.posistion === location) {
                                                    ...
                                                    if (settings !== undefined && settings.notifications === true) notifyRef.current?.message(`${payment_ammount} of money is deducted from the account`, "info", 2, () => {}, false);
                                                    var audio = new Audio("./moneyminus.mp3");
                                                    audio.volume = ((settings?.audio[1] ?? 100) / 100) * ((settings?.audio[0] ?? 100) / 100);
                                                    audio.loop = false;
                                                    audio.play();
                                                    if (prp.morgage === undefined || (prp.morgage !== undefined && prp.morgage === false)) localPlayer.balance -= payment_ammount;
                                                    engineRef.current?.applyAnimation(1);
                                                    socket.emit("pay", {
                                        ...
                                        if (proprety?.id === "incometax") {
                                            ...
                                            if (settings !== undefined && settings.notifications === true) notifyRef.current?.message(`${200} of money is deducted from the account`, "info", 2, () => {}, false);
                                            var audio = new Audio("./moneyminus.mp3");
                                            audio.volume = ((settings?.audio[1] ?? 100) / 100) * ((settings?.audio[0] ?? 100) / 100);
                                            audio.loop = false;
                                            audio.play();
                                        ...
                                        if (proprety?.id === "luxerytax") {
                                            ...
                                            if (settings !== undefined && settings.notifications === true) notifyRef.current?.message(`${100} of money is deducted from the account`, "info", 2, () => {}, false);
                                            var audio = new Audio("./moneyminus.mp3");
                                            audio.volume = ((settings?.audio[1] ?? 100) / 100) * ((settings?.audio[0] ?? 100) / 100);
                                            audio.loop = false;
                                            audio.play();
        ...
        const socket_Unjail = (args: { to: string; option: "card" | "pay" }) => {
            ...
            if (x) {
                if (args.option === "card") {
                    ...
                    x.balance -= 50;
                    if (x.id === socket.id && settings !== undefined && settings.notifications === true) notifyRef.current?.message(`${50} of money is deducted from the account`, "info", 2, () => {}, false);
                    var audio = new Audio("./moneyminus.mp3");
                    audio.volume = ((settings?.audio[1] ?? 100) / 100) * ((settings?.audio[0] ?? 100) / 100);
                    audio.loop = false;
                    audio.play();
            ...
            setTimeout(() => {
                ...
                switch (c.action) {
                    ...
                    case "addfunds":
                        ...
                        if (xplayer.id === socket.id) {
                            if (settings !== undefined && settings.notifications === true) notifyRef.current?.message(`${c.amount ?? 0} of money is added to the account`, "info", 2, () => {}, false);
                            var audio = new Audio("./moneyplus.mp3");
                            audio.volume = ((settings?.audio[1] ?? 100) / 100) * ((settings?.audio[0] ?? 100) / 100);
                            audio.loop = false;
                            audio.play();
                    ...
                    case "removefunds":
                        ...
                        if (xplayer.id === socket.id) {
                            engineRef.current?.applyAnimation(1);
                            if (settings !== undefined && settings.notifications === true) notifyRef.current?.message(`${c.amount ?? 0} of money is deducted from the account`, "info", 2, () => {}, false);
                            var audio = new Audio("./moneyminus.mp3");
                            audio.volume = ((settings?.audio[1] ?? 100) / 100) * ((settings?.audio[0] ?? 100) / 100);
                            audio.loop = false;
                            audio.play();
                                                    ...
                                                    for (const prp of p.properties) {
                                                        if (prp.posistion === location) {
                                                            ...
                                                            if (proprety.group === "Utilities" && prp.rent) {
                                                                ...
                                                                audio.loop = false;
                                                                audio.play();
                                                                if (prp.morgage === undefined || (prp.morgage !== undefined && prp.morgage === false)) xplayer.balance -= payment_ammount;
                                                                engineRef.current?.applyAnimation(1);
                                                                socket.emit("pay", {
                                                                    balance: payment_ammount,
                                                                    from: socket.id,
                    ...
                    case "propertycharges":
                        ...
                        var payment_ammount = (c.buildings ?? 1) * sum(xplayer.properties.filter((v) => typeof v.count === "number").map((v) => v.count as number)) + (c.hotels ?? 1) * xplayer.properties.filter((v) => v.count === "h").length;
                        console.log(`
